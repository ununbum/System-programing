...
int main()
{
	...
	char ins[102];	//get command and trim command
	char history[102];
	...
	for(;;)
	{
		...
		if(strlen(ins)>=100)		// if input is more than 30 clear buffer
			while(getchar()!='\n');
		...
		else if(strcmp(ptr[0],"progaddr")==0)
		{
			if(ptr[1]!=NULL)
				PROGADDR = strtol(ptr[1],&Hex,16);
			else
				PROGADDR = 0;
			Insert_His(history);
		}
		else if(strcmp(ptr[0],"loader")==0)
		{
			if(ptr[1]!=NULL)
			{
				if(Load_function(ptr)==0)
				{
					Insert_His(history);
				}
			}
		}
		else if(strcmp(ptr[0],"run")==0)
		{
			Run_Prog();
		}
		else if(strcmp(ptr[0],"bp")==0)
		{
			Set_bp(ptr[1]);
		}
